...
@Component({
  ...
  styleUrl: './payroll-credit-info.component.scss'
})
export class PayrollCreditInfoComponent implements OnInit {
  ...
  handleFormAction(event: any) {
    ...
    this.hrService.updatePayrollCredit(payload, this.data.data._id).subscribe({
      next: res => {
        //console.log('Update Response', res)
        if(res.success) this.notify.showSuccess('This payroll credit has been updated successfully');
    ...
    :
    this.hrService.createPayrollCredit(payload).subscribe({
      next: res => {
        //console.log('Create Response', res)
        if(res.success) this.notify.showSuccess('Payroll credit was created successfully');
        this.isLoading = false;
        this.emitResponse();
      },
      error: err => {
